...
class PairsDecoder:
    def __init__(self, bit_array, float_format):
        ...
        self.values_decoder = ValuesDecoder(
            bit_array=self.bit_array, float_format=float_format
        )
    ...
    @staticmethod
    def decode_all(content: PairsGorillaContent) -> Iterable[Tuple[int, float]]:
        bit_array = bitarray(endian="big")
        bit_array.frombytes(content["encoded"])

        pairs_decoder = PairsDecoder(bit_array, content["float_format"])

        return [pairs_decoder.decode_next() for _ in range(content["nb_pairs"])]
